...
func (db *Redis) Publish(ctx context.Context, channel string, message interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("failed to marshal message: %v: %w", err, models.ErrInternal)
	}
	if err := db.client.Publish(ctx, channel, data).Err(); err != nil {
		return fmt.Errorf("failed to publish message to redis: %v: %w", err, models.ErrInternal)
	}
	return nil
}
...
func (db *Redis) Orders(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get orders from redis: %v: %w", err, models.ErrInternal)
	}
	return orders, nil
}
...
func (db *Redis) Set(ctx context.Context, key string, value interface{}, expiration time.Duration) error {
	if err := db.client.Set(ctx, key, value, expiration).Err(); err != nil {
		return fmt.Errorf("failed to set key: %v: %w", err, models.ErrInternal)
	}
	return nil
}
...
func (db *Redis) Get(ctx context.Context, key string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to get key: %v: %w", err, models.ErrInternal)
	}
	return value, nil
}
...
func (db *Redis) Del(ctx context.Context, key string) error {
	if err := db.client.Del(ctx, key).Err(); err != nil {
		return fmt.Errorf("failed to delete key: %v: %w", err, models.ErrInternal)
	}
	return nil
}
