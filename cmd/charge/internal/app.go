...
import (
	...
	"cubawheeler.io/cmd/charge/internal/handlers"
...
func (a *App) loader() {
	...
	router.Group(func(r chi.Router) {
		{
			h := handlers.ChargeHandler{
				Service: mongo.NewChargeService(a.mongo),
			}

			router.Route("/v1/charges", func(r chi.Router) {
				r.Get("/", handler(h.Charges))
				r.Get("/{id}", handler(h.Charge))
			})
