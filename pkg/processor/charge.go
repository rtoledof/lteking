...
func (p *Charge) Charge(ctx context.Context, pm cubawheeler.ChargeMethod, amount currency.Amount) (*cubawheeler.Charge, error) {
	charger, err := ChargeRefund(ctx, p.config, pm)
	if err != nil {
		return nil, err
	}
	return charger.Charge(ctx, pm, amount)
}
