// Package qs implements encoding of structs into URL query parameters.
//
// As a simple example:
//
//	type Options struct {
...
// name0=value0&name1=value1, etc. "indexed" will append a number within brackets
// to the the end of each incidence of the value name, example:
// name[0]=value&name[1]=value1, etc. Including the "del" struct tag (separate
		...
		if sv.Kind() == reflect.Slice || sv.Kind() == reflect.Array {
			...
			if del != "" {
				...
				for i := 0; i < sv.Len(); i++ {
					...
					_, ok := sv.Index(i).Interface().(interface{ String() string })
					applyStr := ok && opts.Contains("string")
					if sv.Index(i).Kind() == reflect.Struct && !applyStr {
						if err := reflectValue(values, sv.Index(i), k); err != nil {
							return err
						}
					} else {
